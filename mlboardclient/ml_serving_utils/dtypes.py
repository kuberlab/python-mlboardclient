...
from mlboardclient.ml_serving_utils import types_pb2
...
class DType(object):
  ...
  * `tf.variant`: Values of arbitrary types.
  ...
  """
    ...
    Non-numeric, unordered, and quantized types a
    re not considered unsigned, and this function returns `False`.
    ...
    """
        ...
        # there is no simple way to get the min value of a dtype, we have
        # to check float and int types separately
        try:
            return np.finfo(self.as_numpy_dtype()).min
        # bare except as possible raises by finfo not documented
        except Exception:
            try:
    ...
    """
        ...
        # there is no simple way to get the max value of a dtype,
        # we have to check float and int types separately
        try:
